...
image_dir='/home/nemo/kaggle/data/clean_train_agument_384/'
embedding_size=512
...
train_image_path_list=[join(image_dir,image_name_2_class_name.get(image_name),image_name) for image_name in train_image_name_list]
test_image_path_list=[join(image_dir,image_name_2_class_name.get(image_name),image_name) for image_name in test_image_name_list]
...
def preprocess_function(image_path, image_size):
    file_contents = tf.read_file(image_path[0])
...
def main(args):
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            image_paths_tensor = tf.convert_to_tensor(all_image_paths)
            dataset=tf.data.Dataset.from_tensor_slices(image_paths_tensor)
            dataset = dataset.batch(args.batch_size) 
            dataset.map(lambda x:preprocess_function(x, image_size))
            iterator = dataset.make_initializable_iterator()
            images=iterator.get_next()
                
            evaluate(args, sess, phase_train_placeholder, batch_size_placeholder, embeddings,images, all_image_paths, args.batch_size)
...
def evaluate(args, sess, phase_train_placeholder, batch_size_placeholder, embeddings, images, image_paths, batch_size):
    ...
    embs = np.zeros((nrof_images, embedding_size))
    
    images_placeholder = tf.get_default_graph().get_tensor_by_name("input:0")
    
    print 'nrof_batches',nrof_batches
    for i in range(nrof_batches):
        feed_dict = {phase_train_placeholder:False, batch_size_placeholder:batch_size, images_placeholder: images}
        emb = sess.run([embeddings], feed_dict=feed_dict)
        embs[i*batch_size:(i+1)*batch_size] = emb
    
    left_image_num=nrof_images % batch_size
    feed_dict = {phase_train_placeholder:False, batch_size_placeholder:left_image_num,images_placeholder: images}
    emb = sess.run([embeddings], feed_dict=feed_dict)
    embs[-left_image_num:] = emb
